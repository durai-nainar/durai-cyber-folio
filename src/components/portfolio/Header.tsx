...
export const Header = () => {
  ...
  return (
    <>
      <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 cyber-bg ${isScrolled ? 'bg-background/80 backdrop-blur-md border-b border-border theme-border-header' : 'bg-transparent'}`}>
        <nav className="container mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            {/* Logo */}
            <div className="text-2xl font-bold theme-heading-text">
              Durai's Portfolio
            </div>
            ...
            <div className="hidden md:flex items-center space-x-1 ml-auto mr-4">
              {navItems.map((item, index) => <div key={item.id} className="flex items-center">
                  <button onClick={() => scrollToSection(item.id)} className={`nav-link px-3 py-2 text-lg font-bold transition-all duration-300 hover:scale-110 hover:text-primary ${activeSection === item.id ? 'active text-primary' : 'text-muted-foreground'}`}>
                    {item.label}
                  </button>
